...
Per the non-negotiable engineering rules in [AGENTS.md](AGENTS.md) and [docs/TERSUITE-IMPLEMENTATION-ROADMAP.md](docs/TERSUITE-IMPLEMENTATION-ROADMAP.md):
...
| `OPENHANDS_AGENT_SERVER_API_KEY` | SecretStr | Secret token for Agent Server workspace authentication. Passed exclusively to `OpenHandsRemoteWorkspace.api_key`. |
...
| `OPENROUTER_API_KEY` | SecretStr | Direct provider API key for `openrouter/` models. Passed exclusively to `OpenHandsLLM.api_key`. |
